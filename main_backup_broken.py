...
# Importar configuración de navegación y componentes
    ...
    if st.session_state['processing_complete']:
        ...
        try:
            ...
            if st.button("Validar Estado de Órdenes"):
                ...
                if processable_count == total_count and processable_count > 0:
            ...
            if st.session_state.get('validation_complete', False) and 'orders_status_df' in st.session_state:
                ...
                # Mostrar tabla de resultados con colores
                def highlight_processable(row):
                    if row['Procesable']:
                        return ['background-color: #CCFFCC'] * len(row)  # Verde para procesables
                    else:
                        return ['background-color: #FFCCCC'] * len(row)  # Rojo para no procesables

                st.write("### Estado de las Órdenes:")
                st.dataframe(orders_status_df.style.apply(highlight_processable, axis=1))
                ...
                if st.session_state.get('show_process_button', False):
                    ...
                    if st.button("🚀 Procesar Pagos", type="primary"):
                        ...
                        results = process_payments(models, db_name, uid, odoo_password, processable_df, 
                                                 progress_container, details_container)
